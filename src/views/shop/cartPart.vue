<template>
    <div 
    class="mask" 
    v-if="showCart && calulations.total>0" 
    @click="handleCartShowChange" />
    <div class="cart">

        <div class="product" v-if="showCart && calulations.total>0">
            <div class="product__header">
                <div class="product__header__all" @click="() => setCartItemsChecked(shopId)">
                    <span class="product__header__icon iconfont" v-html="calulations.allChecked ? '&#xe618;':'&#xe619;'"></span>
                    全选
                </div>
        ...
        <div class="check">
            <div class="check__icon">
                <img src="http://www.dell-lee.com/imgs/vue3/basket.png" class="check__icon__img"
                    @click="handleCartShowChange" />
                <div class="check__icon__tag">{{ calulations.total }}</div>
            </div>
            <div class="check__info">
                总计：<span class="check__info__price">&yen; {{ calulations.price }}</span>
            </div>
            <div class="check__btn" v-show="calulations.total > 0">
...
const useCartEffect = (shopId) => {
    ...
    const {productList, changeCartItemInfo,calulations } = useCommonCartEffect(shopId); //购物车相关的就导在这里
    ...
    return {
        calulations, productList, changeCartItemInfo, 
...
export default {
    ...
    setup() {
        ...
        const {
            calulations, productList, cleanCartProducts, 
            changeCartItemInfo, changeCartItemChecked, setCartItemsChecked
        } = useCartEffect(shopId)
        const { showCart, handleCartShowChange } = toggleCartEffect()
        return {
            calulations, shopId, productList, cleanCartProducts,
...
.product {
    ...
    &__header {
        display: flex;
        line-height: .52rem;
        border-bottom: .01rem solid $content-bgClor;
        font-size: .14rem;
